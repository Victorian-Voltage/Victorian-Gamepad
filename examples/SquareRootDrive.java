...
public class SquareRootDrive extends TankDrive {
	...
	@Override protected void configureGamepads() {
		// Configure vertical thumbstick output to follow a square root curve
		// rather than a line (f[x]=x^1/2 rather than f[x]=x^1)
		VictorianGamepad.Function sqrtCurve = new VictorianGamepad.Function() {
			@Override public float func(double x) { return
				(float)(Math.signum(x)*Math.sqrt(Math.abs(x))); }
		};
